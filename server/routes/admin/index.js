module.exports = (app) => {
  ...
  router.get("/", async (req, res) => {
    ...
    if(req.query){
      ...
      console.log('查询条件：', query.customer_id)
      let result = await req.Model.find(query).sort({ create_time: -1 });
      res.send(result);
      return
    }
    let items = await req.Model.find().setOptions(queryOptions).sort({ create_time: -1 }).limit(100);
    res.send(items);
  });
